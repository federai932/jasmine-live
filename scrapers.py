...
def run_all_scrapers():
    # Browser simulato per evitare blocchi nel 2026
    ...
    # --- 1. RANKING SINGOLO (Tuo codice funzionante adattato) ---
    try:
        print("Scraping Singolo...")
        url_s = "https://live-tennis.eu"
        resp_s = scraper.get(url_s, timeout=25)
        # Usiamo 'html.parser' per maggiore compatibilità
        ...
        for df in tabs_s:
            if len(df.columns) >= 10:
                ...
                if any(x in nomi for x in ["Paolini", "Swiatek", "Świątek"]):
                    ...
                    # Selezione colonne ORIGINALE: 0, 3, 5, 6
                    ...
                    with open(os.path.join(STATIC_DIR, "classifica_web.html"), "w", encoding="utf-8") as f:
                        f.write(html)
                    print("OK Singolo salvato!")
                    break
    except Exception as e:
        print(f"Errore Singolo: {e}")

    # --- 2. RANKING DOPPIO (Tuo codice funzionante adattato) ---
    try:
        print("Scraping Doppio...")
        url_d = "https://live-tennis.eu"
        resp_d = scraper.get(url_d, timeout=25)
            ...
            if len(df.columns) >= 8:
                ...
                if any(x in nomi for x in ["Errani", "Paolini", "Hsieh"]):
                    ...
                    with open(os.path.join(STATIC_DIR, "classifica_doppio_web.html"), "w", encoding="utf-8") as f:
                        f.write(html)
                    print("OK Doppio salvato!")
                    break
    except Exception as e:
        print(f"Errore Doppio: {e}")

    # --- 3. NEWS (Tuo codice originale funzionante) ---
    print("Scraping News...")
    notizie = []
    link_visti = set()
    fonti = [
                    ...
                    if link not in link_visti and len(titolo) > 25:
                        ...
                        if img_tag:
                            ...
                            if src and not src.startswith('data:image'): 
                                img_url = requests.compat.urljoin(f['url'], src)
    ...
    # Generazione HTML News
    html_news = '<div class="news-grid">'
    if not notizie:
        html_news += '<p style="color:white; padding:20px;">Nessuna notizia trovata.</p>'
    else:
        for n in notizie[:10]:
            html_news += f'''
    ...
    print("Processo completato correttamente!")
