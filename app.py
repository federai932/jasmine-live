...
def proxy(url):
    ...
    # Pulisce l'URL per evitare errori di protocollo raddoppiato o host mancante
    clean_url = url.replace('https:/', '').replace('http:/', '').replace(':/', '').lstrip('/')
    target_url = f"https://{clean_url}"
    
    try:
        # Simula un browser normale per caricare i contenuti correttamente
        ...
        # Iniezione del tag <base> per non sminchiare la grafica dei fogli di stile (CSS)
        if "wtatennis" in target_url:
            base_tag = f'<base href="https://www.wtatennis.com">'.encode()
